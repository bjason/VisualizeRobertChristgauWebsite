...
function readSentiment(data, error) {
    ...
    function getId(name) {
        return name.replace('*', 'Star').replace('*', 'Star').replace('*', 'Star').replace('+', 'plus').replace('-', 'minus');
    }
    ...
    d3.select('svg g.chart')
        ...
        .on('mouseover', function (d) {
            d3.select('svg g.chart #rankLabel')
                ...
                .duration(500)
                .style('opacity', 1);
            d3.select('svg g.chart #rankLen')
                .text("Average length: " + d.avgLen)
                .transition()
                .duration(500)
                .style('opacity', 1);
            d3.select('svg g.chart #rankPol')
                .text("Polarity: " + d.Polarity)
                .transition()
                .duration(500)
                .style('opacity', 1);
            d3.select('svg g.chart #rankSub')
                .text("Subjectivity: " + d.Subjectivity)
                .transition()
                .duration(500)
                .style('opacity', 1);
        })
        ...
        .on('dblclick', function (d) {
            window.location.assign("list.html?rank=" + d.rank + "&year=all");
        })
        .append("svg:title")
        .text(d => {return "Click to view most frequent words among " + d.rank + " albums.\nDouble click to the lists of " + d.rank + " albums."});
